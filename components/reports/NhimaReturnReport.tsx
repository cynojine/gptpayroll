...
import { getNhimaReturnReportData, getBrandingSettings } from '../../services/api';
import { LoadingSpinner } from '../common/LoadingSpinner';
import { DownloadIcon } from '../icons/IconComponents';

interface NhimaReturnReportProps {
  onBack: () => void;
}
...
export const NhimaReturnReport: React.FC<NhimaReturnReportProps> = ({ onBack }) => {
  ...
  const handleGenerate = async () => {
    ...
    setReportData([]);
    try {
      const data = await getNhimaReturnReportData(month, year);
      if (data.length === 0) {
  ...
  const handleExportCsv = () => {
    const headers = ['Employee Name', 'NRC', 'NHIS ID', 'Contribution (ZMW)'];
    const csvRows = [
      headers.join(','),
      ...reportData.map(row => [
        ...
        row.nrc,
        row.nhisId,
        row.nhimaContribution.toFixed(2),
      ].join(','))
    ];
    ...
    link.setAttribute('download', `NHIMA_Return_${period.replace(' ', '_')}.csv`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  };
  
  const columns: Column<NhimaReturnRow>[] = [
    { header: 'Employee Name', accessor: 'employeeName' },
    { header: 'NRC', accessor: 'nrc' },
    { header: 'NHIS ID', accessor: 'nhisId' },
    { header: 'Contribution', accessor: 'nhimaContribution', render: item => formatCurrency(item.nhimaContribution) },
  ];

  const totalContribution = reportData.reduce((acc, row) => acc + row.nhimaContribution, 0);
    ...
    <Card className="print-container">
      <div className="flex justify-between items-start mb-6 no-print">
        <div>
          ...
          <h2 className="text-xl font-bold text-white">NHIMA Contribution Return</h2>
        </div>
      ...
      <div className="print-only mb-4" style={{ display: 'none' }}>
        <h1 className="text-2xl font-bold">NHIMA Return</h1>
      ...
      {reportData.length > 0 && (
          ...
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-lg font-semibold">NHIMA Return for {period}</h3>
          ...
          <div className="flex justify-end mt-4">
              <div className="w-full md:w-1/3">
                  <div className="flex justify-between p-2 bg-slate-900 font-bold text-lg rounded-lg">
                      <span>Total NHIMA Due:</span>
